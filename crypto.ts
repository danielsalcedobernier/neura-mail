...
export async function GET(request: NextRequest) {
  ...
  const result = await withCronLock('reset_smtp_daily_counters', async () => {
    const reset = await sql`
      UPDATE smtp_servers
      SET sent_today = 0, day_reset_at = NOW() + INTERVAL '1 day'
      WHERE day_reset_at IS NOT NULL AND day_reset_at < NOW()
      RETURNING id
    `
    console.log(`[cron/reset_smtp_daily_counters] Reset ${reset.length} server(s)`)

    await sql`
      UPDATE cron_jobs SET last_run_at = NOW(), last_run_status = 'success', run_count = run_count + 1
      WHERE name = 'reset_smtp_daily_counters'
    `
