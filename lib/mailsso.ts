...
async function getConfig(): Promise<MailssoConfig> {
  ...
  const config = rows[0].extra_config as Record<string, unknown>
  return {
...
export async function verifyEmail(email: string): Promise<VerificationResult> {
  ...
  if (!res.ok) throw new Error(`mails.so API error: ${res.status} ${res.statusText}`)
...
export async function submitBatch(emails: string[]): Promise<string> {
  const config = await getConfig()
  const batchUrl = `${config.baseUrl}/batch`
  const res = await fetch(batchUrl, {
    method: 'POST',
    headers: { 'x-mails-api-key': config.apiKey, 'Content-Type': 'application/json' },
    body: JSON.stringify({ emails }),
  })
  if (!res.ok) {
    const rawText = await res.text().catch(() => res.statusText)
    throw new Error(`mails.so batch error: ${res.status} — ${rawText}`)
  }
  const data = await res.json()
...
export async function pollBatch(batchId: string): Promise<VerificationResult[] | null> {
  ...
  return (data.emails as Record<string, unknown>[]).map(r => mapResult(r.email as string, r))
}
...
export async function checkCacheBulk(emails: string[]): Promise<Map<string, VerificationResult>> {
...
export async function storeBatchInCache(results: VerificationResult[], verifiedByUserId?: string): Promise<void> {
