...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const campaigns = await sql`SELECT * FROM campaigns WHERE id = ${id} AND user_id = ${session.id}`
  ...
  const contacts = await sql`
    SELECT elc.id, elc.email, elc.first_name, elc.last_name
  ...
  const smtpServers = await sql`
    SELECT id FROM smtp_servers WHERE user_id = ${session.id} AND is_active = true LIMIT 10
  `
