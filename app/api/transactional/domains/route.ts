...
// POST to /domains/[id]/verify triggers a DNS check
export async function PUT(request: NextRequest) {
  ...
  const domainRows = await sql`
    SELECT * FROM sending_domains WHERE id = ${domain_id} AND user_id = ${session.id}
  `
  ...
  try {
    // Check TXT record for ownership verification
