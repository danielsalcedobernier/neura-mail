 ...
 * Bulk-inserts pre-verified email results into global_email_cache.
 * Admin only. Accepts up to 5,000 rows per request (browser sends in chunks).
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      return error('rows[] required', 400)
    }
    if (rows.length > 5000) {
      return error('Max 5,000 rows per request', 400)
    }
    ...
    await sql`
      ...
      SELECT
        ...
        (v.verification_score)::numeric,
        (v.mx_found)::boolean,
        (v.smtp_valid)::boolean,
        (v.is_disposable)::boolean,
        (v.is_role_based)::boolean,
        (v.is_catch_all)::boolean,
        v.provider,
        '{}'::jsonb,
        v.verified_by_user_id::uuid,
        (v.expires_at)::timestamptz
