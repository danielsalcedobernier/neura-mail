...
const schema = z.object({
  ...
  broadcast: z.boolean().default(false), // true = send to all users
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (broadcast) {
      // Send to all users — bulk insert
      await sql`
        INSERT INTO notifications (title, message, type, user_id)
        SELECT ${title}, ${message}, ${type}, id FROM users
      `
    } else if (user_id) {
      await sql`
        INSERT INTO notifications (title, message, type, user_id)
        VALUES (${title}, ${message}, ${type}, ${user_id})
      `
    } else {
