...
const STATUS_COLORS = ['text-green-400', 'text-red-400', 'text-yellow-400', 'text-orange-400', 'text-white/40']
const SUB_STATUSES = ['disposable', 'role_based', 'spam_trap', 'mx_not_found', 'smtp_error', 'bounce', 'syntax_error']
const ANCHOR_IDS = ['verify', 'batch', 'credits']
      ...
      <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-12 py-16">
        ...
        <aside className="hidden lg:block w-52 shrink-0">
          <div className="sticky top-24 flex flex-col gap-1">
            <p className="text-xs uppercase tracking-widest text-white/30 font-semibold mb-3">{tr.apiRef}</p>
            {tr.sidebar.map((label, i) => {
              const anchors = ['#auth', '#verify', '#batch', '#credits', '#statuses', '#errors']
              return (
                <a key={label} href={anchors[i]} className="text-sm text-white/40 hover:text-white transition-colors py-1">
                  {label}
                </a>
              )
            })}
            <div className="mt-6 pt-6 border-t border-white/10">
              <Link href="/register">
                <Button size="sm" className="w-full bg-white text-black hover:bg-white/90 gap-1 text-xs">
                  {tr.ctaApiKey} <ArrowRight className="w-3 h-3" />
                </Button>
              </Link>
            </div>
          </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-16">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs uppercase tracking-widest text-primary font-semibold">{tr.apiRef}</span>
            </div>
            <h1 className="text-4xl font-bold text-white">{tr.title}</h1>
            <p className="text-white/40 leading-relaxed max-w-2xl">{tr.intro}</p>
          ...
          <div id="auth" className="flex flex-col gap-5 scroll-mt-24">
            <h2 className="text-2xl font-bold text-white border-b border-white/10 pb-3">{tr.authTitle}</h2>
            <p className="text-sm text-white/50 leading-relaxed">
              {tr.authDesc}{' '}
              <code className="text-white/80 bg-white/5 px-1 rounded">Authorization</code>.{' '}
              {tr.authWhere}{' '}
              <Link href="/dashboard/api" className="text-primary hover:underline">{tr.authKeys}</Link>{' '}
              {tr.authDashboard}
            </p>
            <div className="rounded-xl border border-white/10 bg-black overflow-hidden">
              <div className="flex items-center gap-2 px-5 py-3 border-b border-white/10 bg-white/[0.03]">
                ...
                <span className="text-xs text-white/40 font-mono">{tr.requestHeader}</span>
              </div>
            ...
            <div className="flex items-start gap-3 rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-4">
              ...
              <p className="text-xs text-white/50 leading-relaxed">{tr.authWarning}</p>
            </div>
          </div>

          {/* Endpoints */}
          {tr.endpoints.map((ep, i) => {
            const methodColor = ep.method === 'GET'
              ? 'text-green-400 bg-green-400/10'
              : 'text-blue-400 bg-blue-400/10'
            return (
              <div key={ep.path} id={ANCHOR_IDS[i]} className="flex flex-col gap-5 scroll-mt-24">
                ...
                {ep.params.length > 0 && (
                  <div className="flex flex-col gap-2">
                    <p className="text-xs uppercase tracking-widest text-white/30 font-semibold">{tr.parametersLabel}</p>
                      ...
                      {ep.params.map((p, pi) => (
                        <div key={p.name} className={`flex flex-col sm:flex-row gap-2 sm:items-start p-4 text-sm ${pi > 0 ? 'border-t border-white/5' : ''}`}>
                          ...
                          <div className="flex flex-wrap gap-2 items-center flex-1">
                            ...
                            {p.required && <span className="text-xs px-1.5 py-0.5 rounded bg-red-500/10 text-red-400">{lang === 'es' ? 'requerido' : 'required'}</span>}
                ...
                <div className="flex flex-col gap-2">
                  <p className="text-xs uppercase tracking-widest text-white/30 font-semibold">{tr.responseLabel}</p>
                  <div className="rounded-xl border border-white/10 bg-black overflow-hidden">
                    <div className="flex items-center gap-2 px-4 py-2.5 border-b border-white/10 bg-white/[0.02]">
                      <div className="w-2 h-2 rounded-full bg-green-400/60" />
                      <span className="text-xs text-white/30 font-mono">200 OK · application/json</span>
                    </div>
                    <pre className="p-5 text-xs font-mono text-white/60 overflow-x-auto leading-relaxed">{RESPONSE_EXAMPLES[i]}</pre>
                  </div>
                </div>
              </div>
            )
          ...
          <div id="statuses" className="flex flex-col gap-5 scroll-mt-24">
            <h2 className="text-2xl font-bold text-white border-b border-white/10 pb-3">{tr.statusesTitle}</h2>
            <div className="rounded-xl border border-white/10 overflow-hidden">
              {tr.statuses.map((s, i) => (
                <div key={s.status} className={`flex items-start gap-4 p-4 text-sm ${i > 0 ? 'border-t border-white/5' : ''}`}>
                  <code className={`font-mono font-bold w-24 shrink-0 ${STATUS_COLORS[i]}`}>{s.status}</code>
                  <span className="text-white/50">{s.desc}</span>
                </div>
              ))}
            </div>
            <div className="flex flex-col gap-2">
              <p className="text-xs uppercase tracking-widest text-white/30 font-semibold">{tr.subStatusesLabel}</p>
          ...
          <div id="errors" className="flex flex-col gap-5 scroll-mt-24">
            <h2 className="text-2xl font-bold text-white border-b border-white/10 pb-3">{tr.errorsTitle}</h2>
            <div className="rounded-xl border border-white/10 overflow-hidden">
              {tr.errors.map((e, i) => (
          ...
          <div className="rounded-xl border border-primary/20 bg-primary/5 p-8 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-col gap-1">
              <p className="font-semibold text-white">{tr.readyTitle}</p>
              <p className="text-sm text-white/40">{tr.readySub}</p>
            </div>
            <Link href="/register">
              <Button className="bg-white text-black hover:bg-white/90 shrink-0 gap-2">
                {tr.readyBtn} <ArrowRight className="w-4 h-4" />
              </Button>
            </Link>
          </div>
        </div>
